...
/// 如果图片重导航栏下面算的话  64   如果全屏的话是 0---------不成熟。这个框架更时候不是pop的。
#define kTopHeight 0
...
- (void)viewDidLoad
{
    ...
    if (self.isPush) {
        ...
        self.topView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, 64)];
...
//    UIView *sourceView = self.sourceImagesContainerView.subviews[0];
    /// 下面这三句可以实现返回的时候回到当前的图片位置----超过一屏幕就有点问题了。
    NSIndexPath *indexP = [NSIndexPath indexPathForRow:self.currentPage inSection:0];
    UITableViewCell *cell = [self.topTable cellForRowAtIndexPath:indexP];
    UIView *sourceView = cell.imageView;
...
//            self.navigationController.navigationBarHidden = YES;
//        }
//        NSString *str = @"\"u_a\":0.0000,\"u_b\":0.0000,\"u_c\":0.0000,\"d_a\":0.0000,\"d_b\":0.0000,\"d_c\":0.0000";
...
- (void)setupFrame
{
    ...
    if (self.isPush) {
        ...
        rectSelf.size.height = ScreenHeight;
    }
    ...
    if (self.isPush) {
        self.scrollView.center = CGPointMake(ScreenWidth / 2, ScreenHeight / 2 - kTopHeight/2);
        photoH = ScreenHeight - kTopHeight;
    }
    ...
    if (self.isPush) {
        self.scrollView.contentSize = CGSizeMake(self.scrollView.subviews.count * self.scrollView.width,ScreenHeight - kTopHeight);
    }
    self.scrollView.contentOffset = CGPointMake(self.currentPage * self.scrollView.width, 0);
    
    NSLog(@"y == %f  h ===%f",self.scrollView.origin.y,self.scrollView.size.height);
    CGFloat indexW = 66;
    CGFloat indexH = 28;
...
- (void)setupImageOfImageViewForIndex:(NSInteger)index
{
    ...
    if ([self lowImageURLForIndex:index] && [self lowImageURLForIndex:index] != nil) {
        [photoBrowserView.imageView sd_setImageWithURL:[self lowImageURLForIndex:index] placeholderImage:[self placeholderImageForIndex:index]];
    }
...
#pragma mark - 7.获取低分辨率图片url
- (NSURL *)lowImageURLForIndex:(NSInteger)index
{
    //    self.currentPage = index;
    if ([self.delegate respondsToSelector:@selector(photoBrowser:lowImageURLForIndex:)]) {
        return [self.delegate photoBrowser:self lowImageURLForIndex:index];
    }
    return nil;
}
...
- (UIScrollView *)scrollView
{
    if (!_scrollView) {
        ...
        if (self.isPush) {
            _scrollView.frame = CGRectMake(0, kTopHeight, self.view.bounds.size.width, self.view.bounds.size.height - kTopHeight);
        }
